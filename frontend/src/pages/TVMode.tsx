...
export default function TVMode() {
  ...
  // Ensure theme is applied on mount (fixes white screen on direct navigation)
  useEffect(() => {
    if (isLight) {
  ...
  // Removido auto-rotate (setActivePanel automático) a pedido do usuário
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--color-surface-0)', color: 'var(--color-text-primary)' }}>
      {/* Premium Header */}
      <div className="px-8 py-5 flex items-center justify-between border-b shrink-0" style={{ borderColor: 'var(--border-subtle)', background: 'var(--color-surface-1)' }}>
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-base mt-0.5 tracking-wide uppercase" style={{ color: 'var(--color-elis-teal)' }}>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col text-right">
            ...
            <div className="flex items-center justify-end gap-3 mt-1">
              <span className="w-3 h-3 rounded-full animate-pulse" style={{ background: 'var(--color-status-danger)', boxShadow: '0 0 10px rgba(239,68,68,0.8)' }} />
              <span className="font-mono text-4xl font-bold tracking-wider" style={{ color: 'var(--color-text-primary)' }}>
                {time.toLocaleTimeString('pt-BR')}
              </span>
            </div>
          </div>
          
          <div className="flex items-center gap-3 border-l pl-6" style={{ borderColor: 'var(--border-subtle)' }}>
            <button 
            ...
            >
              <span className="text-2xl">{isLight ? '🌙' : '☀️'}</span>
            </button>
            <button 
              onClick={() => navigate('/')}
              ...
              onMouseEnter={e => {
                e.currentTarget.style.background = 'var(--color-status-danger)'
      ...
      <div className="flex-1 p-8 overflow-hidden flex flex-col">
        {/* Panel toggle buttons — now clickable! */}
        <div className="flex gap-4 mb-6 justify-center">
          <button
            onClick={() => setActivePanel('expedicao')}
            className={`px-8 py-3 rounded-xl text-lg font-bold uppercase tracking-widest transition-all cursor-pointer ${
              activePanel === 'expedicao' ? 'scale-105' : 'opacity-50 hover:opacity-75'
            }`}
            style={{
          ...
          <button
            ...
            className={`px-8 py-3 rounded-xl text-lg font-bold uppercase tracking-widest transition-all cursor-pointer ${
              activePanel === 'producao' ? 'scale-105' : 'opacity-50 hover:opacity-75'
            }`}
            style={{
        ...
        {activePanel === 'expedicao' && (
          <div className="flex-1 flex flex-col" style={{ animation: 'var(--animate-zoom-in)' }}>
            ...
            {/* Departure Board */}
            <div className="flex-1 rounded-2xl overflow-hidden shadow-2xl flex flex-col" style={{ background: 'var(--color-surface-1)', border: '1px solid var(--border-subtle)' }}>
              <div className="grid grid-cols-12 gap-4 p-5 border-b text-lg font-bold uppercase tracking-widest" style={{ borderColor: 'var(--border-subtle)', color: 'var(--color-text-muted)' }}>
              ...
              <div className="flex-1 overflow-auto flex flex-col">
                {expedicoes.length === 0 ? (
                  ...
                  [...expedicoes].sort((a, b) => {
                    ...
                    return (
                      <div key={exp.id} 
                           className="grid grid-cols-12 gap-4 p-5 border-b items-center transition-colors"
                           style={{ 
                        ...
                        <div className="col-span-1 flex justify-center">
                          {isDone ? (
                            <div className="w-10 h-10 rounded-full flex items-center justify-center text-xl font-bold" style={{ background: 'rgba(16,185,129,0.15)', border: '2px solid var(--color-status-ok)', color: 'var(--color-status-ok)' }}>✓</div>
                          ) : isLate ? (
                            <div className="w-10 h-10 rounded-full flex items-center justify-center text-xl font-bold animate-pulse" style={{ background: 'rgba(239,68,68,0.15)', border: '2px solid var(--color-status-danger)', color: 'var(--color-status-danger)' }}>!</div>
                          ) : isWarning ? (
                            <div className="w-10 h-10 rounded-full flex items-center justify-center text-xl font-bold animate-pulse" style={{ background: 'rgba(245,158,11,0.15)', border: '2px solid var(--color-status-warning)', color: 'var(--color-status-warning)' }}>⚠</div>
                          ) : (
                            <div className="w-10 h-10 rounded-full flex items-center justify-center text-xl font-bold" style={{ background: 'rgba(59,130,246,0.15)', border: '2px solid var(--color-status-info)', color: 'var(--color-status-info)' }}>⧗</div>
                          )}
                        </div>
                        <div className="col-span-5 text-2xl font-extrabold truncate" style={{ color: 'var(--color-text-primary)' }}>
                          <button 
                        ...
                        <div className="col-span-2 text-center text-2xl font-mono font-bold" style={{ color: 'var(--color-text-secondary)' }}>
                          {exp.peso_previsto_kg?.toLocaleString('pt-BR')}kg
                        </div>
                        <div className="col-span-2 text-center text-3xl font-mono font-bold" style={{ color: isLate ? 'var(--color-status-danger)' : 'var(--color-text-primary)' }}>
                          {exp.horario_planejado?.slice(0, 5)}
                        </div>
                        <div className="col-span-2 text-center text-3xl font-mono font-bold" style={{ color: isDone ? 'var(--color-status-ok)' : 'var(--color-text-muted)' }}>
        ...
        {activePanel === 'producao' && (
          <div className="flex-1 flex flex-col justify-center max-w-7xl mx-auto w-full" style={{ animation: 'var(--animate-zoom-in)' }}>
            ...
            <div className="p-10 rounded-2xl" style={{ background: 'var(--color-surface-1)', border: '1px solid var(--border-subtle)', boxShadow: 'var(--glass-shadow)' }}>
              <h2 className="text-2xl font-bold mb-12 text-center uppercase tracking-widest" style={{ color: 'var(--color-text-secondary)' }}>
                Rendimento Operacional Instantâneo
              </h2>
              
              <div className="relative mb-10">
                <div className="overflow-visible relative" style={{ height: '80px', borderRadius: '40px', background: 'var(--color-surface-3)', border: '2px solid var(--border-subtle)' }}>
                  <div
                    className="h-full flex items-center justify-end px-8 transition-all duration-1000 ease-in-out"
                    style={{ 
                  ...
                  >
                    <span className="text-white text-4xl font-extrabold drop-shadow-lg font-mono">
                  ...
                  <div className="absolute top-0 bottom-0 flex flex-col justify-center" style={{ left: '94%', zIndex: 10 }}>
                    ...
                    <span className="absolute -top-14 -ml-12 font-bold px-5 py-2 rounded-full text-base tracking-widest border whitespace-nowrap" 
              ...
              <div className="flex justify-between mt-4 px-6 text-2xl font-mono font-bold" style={{ color: 'var(--color-text-muted)' }}>
                <span>0%</span>
                <span>100%</span>
              </div>
            </div>
...
function TVKpi({ label, value, color }: { label: string; value: string | number; color: string }) {
  return (
    <div className="text-center p-6 rounded-2xl" style={{ background: 'var(--color-surface-1)', border: '1px solid var(--border-subtle)' }}>
      <p className="text-sm font-bold mb-3 uppercase tracking-widest" style={{ color: 'var(--color-text-muted)' }}>{label}</p>
      <p className="font-mono font-extrabold text-6xl drop-shadow-md" style={{ color }}>{value}</p>
    </div>
  )
}
